% Guarde num cell array 10.000 sequencias que começam num estado
% não-absorvente (escolhidos com igual probabilidade), e calcule os
% comprimentos mínimos e máximos das sequências geradas.
...
nOfIterations = 10000;
c = cell(1, nOfIterations);
min = 100;
max = 0;
nonAbsorbingStates = [1 2 4];
for i = 1:1:nOfIterations
    ind = randi(3);
    states = crawl(H, nonAbsorbingStates(ind), [3, 5]);
    c{1,i} = states;
    if length(states) > max
        max = length(states);
    end
    if length(states) < min
        min = length(states);
    end
end
